    ...
    return (
        <div>
            ...
            <div className='divRowBetween align-items-start p-5'>
                    ...
                    <div className='border border-primary m-2 formBooking'>
                        ...
                        <div className='divRowBetweenNotAlign'>
                            ...
                            <div className='border border-primary m-2 w-30'>
                                <h5 className='bg-primary w-100 text-center'>Số người tham gia</h5>
                                <div className='px-2'>
                                    <input type='number' className='w-100' min={0} value={numOfChildren} onChange={(e) => setNumOfChildren(e.target.value)} />
                                    <input type='number' className='my-2 w-100' min={1} value={numOfAdults} onChange={(e) => setNumOfAdults(e.target.value)} />
                                    <input type='number' className='w-100' min={0} value={numOfOlds} onChange={(e) => setNumOfOlds(e.target.value)} />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className='w-40 border border-warning m-2 formBooking'>
                    ...
                    <div className='px-3'>
                        ...
                        <Button className='w-100 mb-3' variant="contained"
                            onClick={async () => {
                                const data = await bookingTour({ userId: user.id, departureId: tourDetail.departures[0].departureId, participants: numOfChildren + ',' + numOfAdults + ',' + numOfOlds })
                                if (data)
                                    setNotify(1)
                                else
                                    setNotify(0)
                ...
                <CustomPop notify={notify} onSuccess={bookingSuccess} messageSuccess={"Đặt tour thành công"} onFail={() => setNotify(-1)} />
            </div>
        </div>
    )
}
