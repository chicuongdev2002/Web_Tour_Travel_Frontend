                            ...
                            <tr className="truncate-multiline" key={destination.destinationId}>
                                ...
                                {selectDestination && <td>
                                    <input checked={destinationSelected.content.some(d => d.destinationId == destination.destinationId) ? true : false}
                                        type='checkbox' onChange={(e) => selectDestination(e, destination)} />
                                </td>}
                                {!selectDestination && <td>
                                    <input min={1} style={{ width: 50, backgroundColor: 'white', color: 'black' }} type='number' value={destination.duration} onChange={(e) => changeDuration(e, destination)} />
                                </td>}
                            </tr>
                        ))
                    ) : (
                        <tr>
                            <td colSpan="4" className="text-center">Không có dữ liệu điểm đến.</td>
                        </tr>
                    )}
                </tbody>
                {
