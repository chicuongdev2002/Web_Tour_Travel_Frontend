...
function FormView({
  ...
  return (
    <div
      className={`${notBorder ? "" : "border border-primary"} m-2 formBooking ${className} form-view-container`}
    >
      {title && (
        <div style={{ position: "relative" }}>
          <div
            className="title divCenter"
            style={{ "--titleBackground": titleBackground ?? "lightblue" }}
          >
              ...
              return item.object ? (
                <div key={index}>
                  {item.object.type === "image" ? (
                    ...
                    <SelectComponent
                      label={item.label}
                      listData={item.object.listData}
                      value={item.object.value}
                      onChange={item.object.onChange}
                    />
                  ) : item.object.type === "div" ? (
                    item.object.value
                  ) : (
                    <InputText
                      notForm={item.object.notForm}
                      id={index}
                      label={item.label}
                      ...
                      min={
                        item.object.min !== undefined ? item.object.min : null
                      }
                      className={item.object.className}
                      onChange={item.object.onChange}
                    />
                ...
                <div
                  key={index}
                  className="v-align divRow"
                  style={{ height: 50 }}
                >
                  <div className="divCenter" style={{ width: 30 }}>
                    {item.label.icon}
                  </div>
