...
const TopTours = () => {
  ...
  // Add new refs to track scroll state
  const scrollPositionRef = useRef(0);
  const lastTimeRef = useRef(0);
  const SCROLL_SPEED = 30; // pixels per second
  // Animation keyframes cho hiệu ứng xuất hiện
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await axios.get('http://localhost:8080/api/favorite-tours');
        const sortedTours = response.data.sort((a, b) => b.averageRating - a.averageRating);
...
useEffect(() => {
    ...
    const scroll = (timestamp) => {
      ...
      if (!isHovered) {
        ...
        // When hovered, just update the stored position
  ...
  const getBadgeInfo = (rating) => {
    if (rating >= 4.5) return { text: 'Xuất sắc', color: '#2ecc71', bgColor: '#e8f5e9' };
    if (rating >= 4.0) return { text: 'Rất tốt', color: '#3498db', bgColor: '#e3f2fd' };
    if (rating >= 3.5) return { text: 'Tốt', color: '#f1c40f', bgColor: '#fff3e0' };
    if (rating >= 3.0) return { text: 'Khá', color: '#e67e22', bgColor: '#fff3e0' };
      ...
      {[1, 2, 3].map((item) => (
        <Card key={item} elevation={0} sx={styles.skeletonCard}>
          <CardContent>
            ...
            <Skeleton variant="rectangular" height={8} sx={{ mt: 2, borderRadius: '4px' }} />
          </CardContent>
        </Card>
      ))}
    </Stack>
  ...
  const TourCard = ({ tour, index }) => {
    ...
    const RankBadge = () => {
      if (index === 0) return <Chip icon={<AwardIcon />} label="TOP 1" color="primary" size="small" sx={styles.rankChip1} />;
      if (index === 1) return <Chip icon={<AwardIcon />} label="TOP 2" color="secondary" size="small" sx={styles.rankChip2} />;
      if (index === 2) return <Chip icon={<AwardIcon />} label="TOP 3" color="success" size="small" sx={styles.rankChip3} />;
      return null;
    };
