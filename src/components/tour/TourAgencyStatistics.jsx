  ...
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        ...
        // Tour Type Analysis
        const tourTypeCounts = response.data.reduce((acc, user) => {
        ...
        // Revenue Analysis
        const revenueByType = response.data.map((user) => ({
          userName: user.userName,
          adultRevenue: user.ticketRevenue?.ADULTS || 0,
          childRevenue: user.ticketRevenue?.CHILDREN || 0,
          elderlyRevenue: user.ticketRevenue?.ELDERLY || 0,
        }));
        setRevenueData(revenueByType);

        const revenueByUser = response.data
          .map((user) => ({
            userName: user.userName,
            totalRevenue: user.totalPrice || 0,
            totalTours: user.tourCount,
          }))
                  ...
                  <Bar dataKey="tourCount" name="Số Lượng Tour">
                    {userData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          COLOR_PALETTES.agencyTours[
                            index % COLOR_PALETTES.agencyTours.length
                          ]
                        }
                      />
                    ))}
              ...
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={revenueUser}>
                  ...
                  <Bar dataKey="totalRevenue">
                    {revenueUser.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          COLOR_PALETTES.agencyRevenue[
                            index % COLOR_PALETTES.agencyRevenue.length
                          ]
                        }
                      />
                    ))}
...
export default TourAgencyStatistics;
