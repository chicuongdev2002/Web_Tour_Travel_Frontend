...
function UpdateTourComponent({ data }) {
  ...
  const [destinationSelected, setDestinationSelected] = useState({ content: data.destinations })
  ...
  const convertData = (data) => {
    return data.map((d) => {
      return {
        ...
        childPrice: d.tourPricing.filter(t => t.participantType === 'CHILDREN')[0]?.price,
        adultsPrice: d.tourPricing.filter(t => t.participantType === 'ADULTS')[0]?.price,
        elderlyPrice: d.tourPricing.filter(t => t.participantType === 'ELDERLY')[0]?.price
      }
    })
  }
  ...
  const addButton = (departure) => {
    let d = departure.map(d => {
      return {
        ...d,
        update: <Button className='btn btn-primary bg-success' 
          variant="contained" onClick={() => handleFillUpdateTourModal(d)}>Cập nhật</Button>,
        delete: <Button className='btn btn-danger bg-danger' 
  ...
  const handleUpdateTour = async () => {
    try{
      ...
      if(file){
        ...
        if (!resultUpload) {
            setMessageNotify('Đã xảy ra lỗi khi upload ảnh! Vui lòng thử lại sau')
            setNotification(0)
            return
          }
      }
      let dataRequest = {
        ...
        departures: dataConvert.map(d => {
          return {
            departureId: d.id,
            startDate: moment(d.startDate, 'HH:mm:ss DD/MM/YYYY').format("YYYY-MM-DDTHH:mm:ss"),
            endDate: moment(d.endDate, 'HH:mm:ss DD/MM/YYYY').format("YYYY-MM-DDTHH:mm:ss"),
            maxParticipants: d.maxParticipants,
          }
        }),
        tourPricing: dataConvert.map(d => {
          return {
            departure: {
              departureId: d.id
            },
      ...
      if(putData(UPDATE_TOUR, dataRequest))
        setNotification(1)
      else
        setNotification(0)
    } catch (error) {
  ...
  const handleFillUpdateTourModal = (departure) => {
    ...
    setStartDate(moment(departure.startDate, 'HH:mm:ss DD/MM/YYYY').format('HH:mm:ss DD/MM/YYYY'))
    setEndDate(moment(departure.endDate, 'HH:mm:ss DD/MM/YYYY').format('HH:mm:ss DD/MM/YYYY'))
  ...
  const handleChangeHourEndDate = (hour) => {
    ...
    let endDate = moment(d.endDate, 'HH:mm:ss DD/MM/YYYY').add(hour, 'hours').format('HH:mm:ss DD/MM/YYYY')
      return {
        ...d,
        endDate: endDate
      }
  ...
  const handleUpdateDeparture = () => {
    setDataConvert(addButton(dataConvert.map(d => {
      if (d.id === departureId) {
  ...
  return (
    <div>
      <div className={`w-100 ${openDestination ? "divRowBetweenNotAlign" : "divCenter"}`}>
        ...
        ]} />
        {
          ...
          <FormView title='Chọn địa điểm' className='w-50'>
            <div>
              <DestinationList duration={duration} data={destinationSelected} isDescription={false} onGetData={getDestination}
      ...
      {
        openDeparture && 
        <TableComponent headers={['Id', 'Ngày khởi hành', 'Ngày kết thúc', 'Số người tham gia', 
          'Giá vé trẻ em', 'Giá vé người lớn', 'Giá vé người già', {
            colSpan: 2,
            object: <Button className='btn btn-primary' variant="contained" onClick={handleFillAddTourModal}>Thêm</Button>
          }]} data={dataConvert} />
      }
      {
        updateDeparture &&
        <ModalComponent open={updateDeparture} onclose={() => setUpdateDeparture(false)} title='Cập nhật lịch trình'>
          <FormView notBorder={true}>
            <div style={{ marginTop: -20 }}>
              <InputText label='Id' disable={true} type="text" value={departureId} />
              <InputText label='Ngày khởi hành' type="dateTime" value={startDate} onChange={(e) => {
                setStartDate(e.target.value)
                setEndDate(moment(e.target.value, "HH:mm:ss DD/MM/YYYY").add(duration, 'hours').format('HH:mm:ss DD/MM/YYYY'))
              }} />
              <InputText label='Ngày kết thúc' disable={true} type="dateTime" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
              <InputText label='Số người tham gia' type="number" value={maxParticipants} min={1} onChange={(e) => setMaxParticipants(e.target.value)} />
              <InputText label='Thời gian(giờ)' disable={true} type="number" value={duration} />
              <div className="divRowBetween">
                <div className="w-30">
                  <InputText label='Giá vé trẻ em' type="number" value={childrenPrice} min={0}
                    onChange={(e) => setChildrenPrice(e.target.value)} />
                </div>
                <div className="w-30">
                  <InputText label='Giá vé người lớn' type="number" value={adultPrice} min={1}
                    onChange={(e) => setAdultPrice(e.target.value)} />
                </div>
                <div className="w-30">
                  <InputText label='Giá vé người già' type="number" value={oldPrice} min={0}
                    onChange={(e) => setOldPrice(e.target.value)} />
                </div>
              </div>
              <Button className='btn btn-primary w-100 mb-3 mt-3' variant="contained"
                onClick={handleUpdateDeparture}>Cập nhật</Button>
            </div>
          </FormView>
        </ModalComponent>
      }
      <ModalComponent open={openAddDeparture} onclose={() => setOpenAddDeparture(false)} title='Thêm lịch trình'>
        <FormView notBorder={true}>
          <div style={{ marginTop: -20 }}>
            <InputText label='Id' disable={true} type="text" value={departureId} />
            <InputText label='Ngày khởi hành' type="dateTime" value={startDate} onChange={(e) => {
            ...
            <Button className='btn btn-primary w-100 mb-3 mt-3' variant="contained"
              onClick={handleAddDeparture}>Thêm</Button>
          </div>
        </FormView>
      </ModalComponent>
      <ChoosePopup open={notification === -2} onclose={() => setNotification(-1)} title='Xoá lịch trình' message='Bạn có chắc chắn muốn xoá lịch trình này?' 
          onAccept={() => { setNotification(-1); handleDeleteTour(departureSelected) }} onReject={() => setNotification(-1)}
        />
      <Button className='btn btn-primary w-50 mt-3' variant="contained"
        onClick={handleUpdateTour}>Cập nhật tour</Button>
      <CustomPop notify={notification} onSuccess={ () => {
        navigate('/tour-details/' + data.tourId)
        setNotification(-1) }} messageSuccess="Cập nhật tour thành công" 
