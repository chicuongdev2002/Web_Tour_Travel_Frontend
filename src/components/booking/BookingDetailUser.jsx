...
const BookingDetailUser = () => {
  ...
  const convertParticipants = (participantsString) => {
    ...
    return participants.reduce((acc, count, index) => {
      if (count > 0) {
  ...
  {booking.tourGuide ? booking.tourGuide.fullName : "Không xác định"}
                    ...
                    <TableCell sx={{ whiteSpace: 'nowrap' }}>
                      <Grid container alignItems="center" spacing={1}>
                        ...
                        <Grid item>
                          {booking.payments.reduce((sum, payment) => sum + payment.amount, 0).toLocaleString()} VND
                        </Grid>
                      </Grid>
                    </TableCell>
                    <TableCell sx={{ whiteSpace: 'nowrap' }}>
                      {booking.payments.map(payment => (
                        <Chip 
                          ...
                          label={paymentMethodDescriptions[payment.paymentMethod].label}
                          color={paymentMethodDescriptions[payment.paymentMethod].color}
                          size="small"
                          variant="soft"
                          sx={{ 
                            margin: 0.5,
                    ...
                    <TableCell sx={{ whiteSpace: 'nowrap' }}>
                      <Chip 
                        icon={
                          booking.checkinStatus === 'CHECKED_IN' ? <CheckCircleIcon /> : <ErrorIcon />
                        }
                        label={checkInStatusDescriptions[booking.checkinStatus].label}
                        color={checkInStatusDescriptions[booking.checkinStatus].color}
                        variant="soft"
                        size="small"
                        sx={{
                          fontWeight: 600,
