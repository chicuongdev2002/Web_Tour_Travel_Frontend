...
const translateParticipantType = (type) => {
  ...
  tileClassName={({ date }) => {
    ...
    const startDate = normalizeDate(new Date(selectedDeparture.startDate));
    const endDate = normalizeDate(new Date(selectedDeparture.endDate));
...
/>
            ...
            <div className="date-range">
              <div className="date-highlight">
                <strong>Ngày khởi hành:</strong> {new Date(selectedDeparture.startDate).toLocaleDateString('vi-VN')}
              </div>
              <div className="date-highlight">
                <strong>Ngày kết thúc:</strong> {new Date(selectedDeparture.endDate).toLocaleDateString('vi-VN')}
              </div>
            </div>
            ...
            <div className="pricing-list">
              ...
              {selectedDeparture.tourPricing.map((pricing, index) => (
                <div key={index} className="participant-info">
                  <span>{translateParticipantType(pricing.participantType)}:</span>
                  <span>{pricing.price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
                </div>
              ))}
            </div>
