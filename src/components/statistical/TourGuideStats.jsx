...
const TourGuideStats = () => {
 ...
 const calculateAverageHours = (workingHours) => {
  ...
  const totalHours = workingHours.reduce((sum, guide) => sum + guide.totalHours, 0);
...
const fetchData = async () => {
  try {
    ...
    const [statsResponse, hoursResponse] = await Promise.all([
      ...
      fetch(`http://localhost:8080/api/tour-guides/working-hours?startDate=${startISO}&endDate=${endISO}`)
    ]);
...
const handleTimeRangeChange = (event, newValue) => {
 ...
 const averageHours = calculateAverageHours(workingHours);
  ...
  const calculateMaxY = (data) => {
    ...
    const actualTotal = (data.totalAccepted || 0) + 
                       (data.totalRejected || 0) + 
                       (data.totalTodo || 0);
    return actualTotal;
  };
  const statusData = [
    { name: 'Chờ chấp nhận', value: data.todoRate || 0, color: theme.palette.warning.main },
    { name: 'Đồng ý', value: data.acceptanceRate || 0, color: theme.palette.success.main },
    { name: 'Từ chối', value: data.rejectionRate || 0, color: theme.palette.error.main }
  ].filter(item => item.value > 0);

  const completionData = [
    { name: 'Hoàn thành', value: data.completionRate || 0, color: theme.palette.primary.main },
    { name: 'Còn lại', value: 100 - (data.completionRate || 0), color: theme.palette.grey[200] }
  ];
  ...
  const sortedWorkingHours = [...workingHours].sort((a, b) => b.totalHours - a.totalHours);
  ...
  const CardActions = () => (
    <>
      ...
      >
        ...
        <MenuItem onClick={() => { handleMenuClose(); fetchData(); }}>
